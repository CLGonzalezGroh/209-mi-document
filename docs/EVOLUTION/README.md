...
| `BLOCK_03B_TITULARIDAD_POR_NIVEL.md` | Qué le pertenece a cada nivel: metadata de identificación en la revisión, código inmutable con acto de reemplazo N:M entre documentos, la versión como conjunto de archivos con rol producido por copia de trabajo, y una palabra por nivel para los estados terminales | `PROMOVIDO_A_SFS` |
| `BLOCK_04_EMISION_Y_RESPUESTA.md` | Cuarto bloque: qué le pasa al documento cuando hay contraparte. Naturaleza del transmittal y sentido derivado del rol, puerta dura de emisión, propósito con comportamiento, la respuesta como objeto propio del ítem con archivos y autoría diferenciada, acuse de recibo, catálogo de calificaciones, circuito del rol Receptor y documento pendiente derivado | `PROMOVIDO_A_SFS` |
| `BLOCK_02B_UBICACION_FISICA.md` | Ubicación física jerárquica del documento, y el mecanismo de alcance por proyecto que los tres catálogos documentales comparten: herencia del despliegue con ampliación, catálogo propio y siembra por copia. Descarta el eje de área de la matriz de responsabilidad | `PROMOVIDO_A_SFS` |
| `BLOCK_02C_ALCANCE_POR_PROYECTO.md` | Alcance por proyecto de clase y tipo, aplicando el mecanismo que construyó `BLOCK_02B`: clase y tipo declaran juntos, siembra conjunta con identidad por código dentro de la clase, cruce en un solo sentido, y el eje de módulo preparado en la declaración de alcance. Primer bloque sobre objetos con datos e interfaz en producción | `PROMOVIDO_A_SFS` |
| `BLOCK_02D_RAIZ_DE_ALCANCE_PROPIA.md` | El contrato como entidad del módulo: `DocProject` con identidad propia, `DocProjectSettings` disuelto dentro suyo, vínculo opcional y N:1 con la gestión PMI de `mi-project` —que habilita varios contratos por obra—, contraparte como referencia a `Company`, y unicidad del código discriminada por módulo. Desbloquea la ubicación definitiva de `BLOCK_05` | `APROBADO_PENDIENTE` |
